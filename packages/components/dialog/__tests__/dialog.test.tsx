...
import triggerCompositeClick from '@teamway-ui/test-utils/composite-click'
...
describe('Dialog.vue', () => {
  test('render test', async () => {
    ...
    await rAF()
    await nextTick()
    expect(wrapper.find('.ty-dialog__body').text()).toEqual(AXIOM)
  })
  ...
  test('dialog should have a title and header when it has been given', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.ty-dialog__header').text()).toBe(HEADER)
  ...
  test('dialog should have a footer when footer has been given', async () => {
    ...
    expect(wrapper.find('.ty-dialog__footer').text()).toBe(AXIOM)
  })
  ...
  test('should append dialog to body when appendToBody is true', async () => {
    ...
    await nextTick()
    expect(
      document.body.firstElementChild!.classList.contains('ty-overlay')
    ).toBe(true)
    wrapper.unmount()
  })
  ...
  test('should center dialog', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.ty-dialog--center').exists()).toBe(true)
  })
  ...
  test('should show close button', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.ty-dialog__close').exists()).toBe(true)
  })
  ...
  test('dialog content should not be clickable during close animation', async () => {
    ...
    const overlayDialog = wrapper.find('.ty-overlay-dialog')
  ...
  describe('mask related', () => {
    ...
    test('should close the modal when clicking on mask when `closeOnClickModal` is true', async () => {
      ...
      await nextTick()
      expect(wrapper.find('.ty-overlay').exists()).toBe(true)
      expect(wrapper.find('.ty-overlay-dialog').exists()).toBe(true)

      await triggerCompositeClick(wrapper.find('.ty-overlay-dialog'))
      expect(wrapper.vm.visible).toBe(false)
    })
  ...
  describe('life cycles', () => {
    ...
    test('should destroy on close', async () => {
      ...
      await rAF()
      await nextTick()
      expect(wrapper.find('.ty-dialog__body').exists()).toBe(false)
    })
  ...
  describe('accessibility', () => {
    ...
    test('missing title attribute should point to header slot content', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogTitle = wrapper.find('.ty-dialog__title')
    ...
    test('aria-describedby should point to modal body', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogBody = wrapper.find('.ty-dialog__body')
