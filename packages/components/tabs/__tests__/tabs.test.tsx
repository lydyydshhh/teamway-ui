...
import type { TabPaneName, TabsPaneContext } from '@teamway-ui/components/tabs'
...
describe('Tabs.vue', () => {
  test('create', async () => {
    ...
    const navItemsWrapper = navWrapper.findAll('.ty-tabs__item')

    expect(navItemsWrapper[0].classes('is-active')).toBe(true)
    expect(panesWrapper[0].classes('ty-tab-pane')).toBe(true)
  ...
  test('active-name', async () => {
    ...
    expect(panesWrapper[1].classes('ty-tab-pane')).toBe(true)
  ...
  test('default-value', async () => {
    ...
    let navItemsWrapper = navWrapper.findAll('.ty-tabs__item')
    expect(navItemsWrapper[1].classes('is-active')).toBe(true)
    expect(tabsWrapper.vm.$.exposed!.currentName.value).toEqual('b')
    ...
    navItemsWrapper = navWrapper.findAll('.ty-tabs__item')
    expect(navItemsWrapper[2].classes('is-active')).toBe(true)
    expect(tabsWrapper.vm.$.exposed!.currentName.value).toEqual('c')
  })
  ...
  test('card', async () => {
    ...
    const tabsWrapper = wrapper.findComponent(Tabs)
    expect(tabsWrapper.classes('ty-tabs--card')).toBe(true)
  })
  ...
  test('border card', async () => {
    ...
    const tabsWrapper = wrapper.findComponent(Tabs)
    expect(tabsWrapper.classes('ty-tabs--border-card')).toBe(true)
  })
  ...
  test('editable', async () => {
    ...
    await wrapper.find('.ty-tabs__new-tab').trigger('click')
  ...
  test('tab order', async () => {
    ...
    const items = wrapper.findAll('.ty-tabs__item')
  ...
  test('tab-position', async () => {
    ...
    expect(tabsWrapper.classes('ty-tabs--left')).toBe(true)
    expect(tabsWrapper.find('.ty-tabs__header').classes('is-left')).toBe(true)
    expect(tabsWrapper.find('.ty-tabs__nav-wrap').classes('is-left')).toBe(true)
    expect(tabsWrapper.find('.ty-tabs__nav').classes('is-left')).toBe(true)
    expect(tabsWrapper.find('.ty-tabs__active-bar').classes('is-left')).toBe(
      true
    )
    expect(tabsWrapper.find('.ty-tabs__item').classes('is-left')).toBe(true)
  })
  ...
  test('stretch', async () => {
    ...
    expect(tabsWrapper.find('.ty-tabs__nav').classes('is-stretch')).toBe(true)

    tabPosition.value = 'left'
    await nextTick()

    expect(tabsWrapper.find('.ty-tabs__nav').classes('is-stretch')).toBe(false)
  })
  ...
  test('tab active bar offset', async () => {
    ...
    await nextTick()
    expect(tabsWrapper.find('.ty-tabs__active-bar').attributes().style).toMatch(
      'translateX(300px)'
    )
  ...
  test('should work with lazy', async () => {
    ...
    expect(wrapper.findAll('.ty-tab-pane').length).toBe(3)

    await navItemsWrapper[3].trigger('click')

    expect(wrapper.findAll('.ty-tab-pane').length).toBe(4)
  })
