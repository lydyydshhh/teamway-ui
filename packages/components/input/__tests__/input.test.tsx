...
describe('Input.vue', () => {
  ...
  describe('test emoji', () => {
    test('ty-input should minimize value between emoji length and maxLength', async () => {
      ...
      const elCount = wrapper.find('.ty-input__count-inner')
    ...
    test('textarea should minimize value between emoji length and maxLength', async () => {
      ...
      const elCount = wrapper.find('.ty-input__count')
      expect(elCount.exists()).toBe(true)
      expect(elCount.text()).toMatchInlineSnapshot(`"4 / 4"`)
  ...
  test('suffixIcon', () => {
    ...
    const icon = wrapper.find('.ty-input__icon')
  ...
  test('size', () => {
    ...
    expect(wrapper.classes('ty-input--large')).toBe(true)
  })

  test('type', () => {
    const wrapper = mount(() => <Input type="textarea" />)
    expect(wrapper.classes('ty-textarea')).toBe(true)
  })
  ...
  test('limit input and show word count', async () => {
    ...
    expect(inputElm1.querySelectorAll('.ty-input__count').length).toEqual(0)
    expect(inputElm2.querySelectorAll('.ty-input__count').length).toEqual(1)
    expect(inputElm3.querySelectorAll('.ty-input__count').length).toEqual(0)
    ...
    show.value = true
    await nextTick()
    expect(inputElm1.querySelectorAll('.ty-input__count').length).toEqual(1)
  ...
  describe('Input Events', () => {
    ...
    test('event:clear', async () => {
      ...
      await input.trigger('focus')
      await nextTick()
      vm.$el.querySelector('.ty-input__clear').click()
      await nextTick()
  ...
  test('show-password icon', async () => {
    ...
    const icon = wrapper.find('.ty-input__icon.ty-input__password')
  ...
  describe('form item accessibility integration', () => {
    ...
    test('The disabled state of a component has higher priority than that of a form', async () => {
      ...
      await nextTick()
      const input = wrapper.find('.ty-input')
  ...
  test('textarea-show-word-limit-outside-position', async () => {
    ...
    const wordLimit = wrapper.find('.ty-input__count')
    await nextTick()
