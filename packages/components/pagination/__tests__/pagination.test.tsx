...
const assertCurrent = (wrapper: VueWrapper<any>, page: number) => {
  expect(wrapper.find('.ty-pager li.is-active.number').text()).toBe(
    String(page)
  )
}
const assertPages = (wrapper: VueWrapper<any>, total: number) => {
  expect(wrapper.find('.ty-pagination .ty-pager li:last-child').text()).toBe(
    String(total)
  )
}
...
describe('Pagination', () => {
  describe('test invalid usages', () => {
    ...
    test('both absence of total & pageCount is invalid', async () => {
      ...
      expect(wrapper.find('.ty-pagination').exists()).toBe(false)
      expect(console.warn).toHaveBeenCalled()
      total.value = 100
      await nextTick()
      expect(wrapper.find('.ty-pagination').exists()).toBe(true)
    })
  ...
  describe('test layout & layout reactive change', () => {
    ...
    const layoutSelectorPairs = [
      ['sizes', '.ty-pagination__sizes'],
      ['prev', 'button.btn-prev'],
      ['pager', 'ul.ty-pager'],
      ['next', 'button.btn-next'],
      ['jumper', '.ty-pagination__jump'],
      ['total', '.ty-pagination__total'],
    ]
    ...
    test(`layout with '->, total'`, async () => {
      ...
      assertElementsExistence(
        wrapper,
        ['.ty-pagination__total', '.ty-pagination__rightwrapper'],
        true
      )
    })
    ...
    test('test small layout', () => {
      ...
      expect(wrapper.vm.$el.classList.contains('ty-pagination--small')).toBe(
        true
      )
    })
  ...
  describe('test pageSize & currentPage reactive change', () => {
    ...
    test('test currentPage change', async () => {
      ...
      await wrapper.find('.ty-pager li:last-child').trigger('click')
    ...
    test('test more icon', async () => {
      ...
      await wrapper.find('.ty-pager li:nth-child(5)').trigger('click')
      assertCurrent(wrapper, 5)
      expect(wrapper.find('.btn-quickprev').exists()).toBeTruthy()
      expect(wrapper.findComponent(DArrowLeft).exists()).toBeFalsy()
      ...
      await wrapper.find('.ty-pager li:nth-child(3)').trigger('click')
      assertCurrent(wrapper, 5)
      expect(wrapper.find('.btn-quicknext').exists()).toBeTruthy()
      expect(wrapper.findComponent(DArrowRight).exists()).toBeFalsy()
    ...
    test('test listener work', async () => {
      ...
      expect(count).toBe(1)
      await wrapper.find('.ty-select').trigger('click')
      await wrapper
  ...
  describe('test a11y supports', () => {
    test('test a11y attributes', async () => {
      ...
      const prev = wrapper.find('.ty-pagination .btn-prev')
      const next = wrapper.find('.ty-pagination .btn-next')
      const pagers = wrapper.findAll('.ty-pagination .ty-pager .number')
    ...
    test('test tabindex interactive', async () => {
      const wrapper = mount(() => <Pagination total={100} />)
      await wrapper.find('.ty-pager li:nth-child(2)').trigger('click')
      assertCurrent(wrapper, 2)
      await wrapper.find('.ty-pager li:nth-child(3)').trigger('click', {
        key: 'Enter',
      })
      ...
      /*
       * await wrapper.find('.ty-pager li:nth-child(3)').trigger('keyup', {
       *   key: 'Tab',
       * })
       * const style = window.getComputedStyle(wrapper.find('.ty-pager li:nth-child(4)').element, ':focus-visible')
    ...
    test('test tabindex disabled', async () => {
      ...
      expect(
        wrapper.find('.ty-pager li:first-child').attributes('tabindex')
      ).toBe('-1')
