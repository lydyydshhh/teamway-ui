...
describe('TimeLine.vue', () => {
  test('create', () => {
    ...
    const contentWrappers = wrapper.findAll('.ty-timeline-item__content')
  ...
  test('placement', () => {
    ...
    const timestampWrapper = wrapper.findAll('.ty-timeline-item__timestamp')[0]
  ...
  describe('mode', () => {
    test('mode-start', () => {
      ...
      const timeline = wrapper.find('.ty-timeline')
      const timelineItems = wrapper.findAll('.ty-timeline-item')
  ...
  test('hide-timestamp', () => {
    ...
    const timestampWrappers = wrapper.findAll('.ty-timeline-item__timestamp')
  ...
  test('color', () => {
    ...
    const vm = wrapper.vm
    const nodeElm = vm.$el.querySelector('.ty-timeline-item__node')
  ...
  test('type', () => {
    ...
    const nodeWrapper = wrapper.find('.ty-timeline-item__node')

    expect(nodeWrapper.classes('ty-timeline-item__node--primary')).toBe(true)
  })
  ...
  test('size', () => {
    ...
    expect(nodeWrapper.classes('ty-timeline-item__node--large')).toBe(true)
  })
  ...
  test('icon', () => {
    ...
    expect(wrapper.find('.ty-timeline-item__icon').exists()).toBe(true)
  ...
  test('dot', () => {
    ...
    const dotWrapper = wrapper.find('.ty-timeline-item__dot')

    expect(dotWrapper.text()).toEqual('dot')
    expect(wrapper.find('.ty-timeline-item__node').exists()).toBe(false)
  })
  ...
  test('center', () => {
    ...
    const timestampWrappers = wrapper.findAll('.ty-timeline-item')

    expect(timestampWrappers[1].classes()).toContain('ty-timeline-item__center')
  })
  ...
  describe('reverse', () => {
    test('v-for children', async () => {
      ...
      let firstTimelineItem = wrapper.find('.ty-timeline-item__content')
      expect(firstTimelineItem.text()).toMatchInlineSnapshot(`"Step 3: xxxxxx"`)

      await wrapper.setData({ reverse: false })
      firstTimelineItem = wrapper.find('.ty-timeline-item__content')
