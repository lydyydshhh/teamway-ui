...
describe('Color-picker-panel', () => {
  it('Mount Color-picker-panel', () => {
    ...
    expect(wrapper.html()).toContain('ty-color-picker')
    wrapper.unmount()
  })
  ...
  describe('disabled', () => {
    ...
    it('predefine color disabled', async () => {
      await wrapper.find('.ty-color-predefine__color-selector').trigger('click')
      expect(updateModelValue).not.toHaveBeenCalled()
    })
  ...
  it('should show alpha slider when show-alpha=true', async () => {
    ...
    const alphaSlider = wrapper.find('.ty-color-alpha-slider')
  ...
  it('control alpha changes through keyboard', async () => {
    ...
    const alphaSlider = wrapper.findComponent('.ty-color-alpha-slider')
    await alphaSlider.find('.ty-color-alpha-slider__thumb').trigger('keydown', {
      key: EVENT_CODE.down,
  ...
  it('should change color to the selected color', async () => {
    ...
    expect(
      predefineDom.querySelectorAll('.ty-color-predefine__color-selector')
        .length === 9
    ).toBeTruthy()
    predefineDom
      .querySelector<HTMLElement>(
        '.ty-color-predefine__color-selector:nth-child(4)'
      )
      ?.click()
    await nextTick()
    ...
    predefineDom
      .querySelector<HTMLElement>(
        '.ty-color-predefine__color-selector:nth-child(3)'
      )
      ?.click()
    await nextTick()
  ...
  it('should change selected state of predefined color', async () => {
    ...
    expect(
      predefineWrapper
        .find('.ty-color-predefine__color-selector:nth-child(4)')
        .classes()
    ).toContain('selected')
  ...
  it('should update the colorFormat and selected color when the colorFormat prop changes', async () => {
    ...
    expect(
      customInput.find<HTMLInputElement>('.ty-input__inner').element.value
    ).toBe('rgb(0, 255, 0)')
    wrapper.unmount()
  })
  ...
  it('control hue changes through keyboard', async () => {
    ...
    const alphaSlider = wrapper.findComponent('.ty-color-hue-slider')
    await alphaSlider.find('.ty-color-hue-slider__thumb').trigger('keydown', {
      key: EVENT_CODE.down,
      code: EVENT_CODE.down,
    })
    await alphaSlider.find('.ty-color-hue-slider__thumb').trigger('keydown', {
  ...
  it('control saturation and brightness changes through keyboard', async () => {
    ...
    const cursor = wrapper.find('.ty-color-svpanel__cursor')
  ...
  it('predefine colors should show different color when showAlpha values are different', async () => {
    ...
    const bg = wrapper.find('.ty-color-predefine__color-selector div')
  ...
  describe('a11y label', () => {
    it('default', async () => {
      const color = ref('#409eff')
      const wrapper = mount(() => <ColorPickerPanel v-model={color.value} />)
      const svPanel = wrapper.find('.ty-color-svpanel__cursor')
      ...
      const hueSlider = wrapper.find('.ty-color-hue-slider__thumb')
    ...
    it('with show-alpha', async () => {
      ...
      const alphaSlider = wrapper.find('.ty-color-alpha-slider__thumb')
    ...
    it('with predefine', async () => {
      ...
      const predefineColor = wrapper.find('.ty-color-predefine__color-selector')
