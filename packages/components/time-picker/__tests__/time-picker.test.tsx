...
describe('TimePicker', () => {
  ...
  it('set format && default value && set AM/PM spinner && no $attr to panel', async () => {
    ...
    const items = hoursEl.querySelectorAll('.ty-time-spinner__item')
    ...
    const times = document.querySelectorAll('.ty-time-spinner__list .is-active')
    expect(times[0].textContent).toBe('06 PM')
    expect(times[1].textContent).toBe('40') // default value
    expect(times[2].textContent).toBe('00')
    const panel = document.querySelector('.ty-time-panel') as any
  ...
  it('select time', async () => {
    ...
    const hourEl = hoursEl.querySelectorAll('.ty-time-spinner__item')[4] as any
    const minuteEl = minutesEl.querySelectorAll(
      '.ty-time-spinner__item'
    )[36] as any
  ...
  it('click confirm / cancel button', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.ty-time-panel__btn.cancel') as any).click()

    expect(value.value).toBe('')
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.ty-time-panel__btn.confirm') as any).click()
  ...
  it('should correctly cancel value after opened twice', async () => {
    ...
    await input.trigger('blur')
    await input.trigger('focus')
    await picker.find('.ty-time-panel__btn.cancel').trigger('click')
  ...
  it('set format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const spinnerDom = document.querySelectorAll('.ty-time-spinner__wrapper')
  ...
  it('should cancel correctly the right value after manual input', async () => {
    ...
    const secondPanel = picker.findAll('.ty-time-range-picker__cell')[1]
    const currentHour = secondPanel.find('.ty-time-spinner__item.is-active')
    expect(currentHour.text()).toBe('09')
    await currentHour.trigger('mouseover')
    await picker.find('.ty-time-panel__btn.cancel').trigger('click')
    const secondInput = wrapper.findAll('.ty-range-input')[1]
  ...
  it('selectableRange ', async () => {
    ...
    const hourSpinners = hoursEl.querySelectorAll('.ty-time-spinner__item')
...
describe('TimePicker(range)', () => {
  it('create', async () => {
    ...
    expect(wrapper.find('.ty-range-editor--small').exists()).toBeTruthy()
    const input = wrapper.find('input')
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ...
    const list = document.querySelectorAll(
      '.ty-time-spinner__list .ty-time-spinner__item.is-active'
    )
  ...
  it('cancel button', async () => {
    ...
    expect(document.querySelector('.ty-picker-panel')).toBeNull()
    input.trigger('blur')
    input.trigger('focus')
    await nextTick()
    ;(document.querySelector('.ty-time-panel__btn.confirm') as any).click()
  ...
  it('clear button', async () => {
    ...
    const findInputWrapper = () => wrapper.find('.ty-date-editor')
    const findClear = () => wrapper.find('.ty-range__close-icon')
  ...
  it('should close pick when click the clear button on pick opened', async () => {
    ...
    const findInputWrapper = () => wrapper.find('.ty-date-editor')
    const findClear = () => wrapper.find('.ty-range__close-icon')
    const findPicker = () => wrapper.find('.ty-picker-panel')
  ...
  it('selectableRange ', async () => {
    ...
    ;(leftHoursEl.querySelectorAll('.ty-time-spinner__item')[12] as any).click()
    await nextTick()
  ...
  it('am/pm mode avoid render redundant content', async () => {
    ...
    expect(
      list[0]
        .querySelector('.ty-time-spinner__item.is-active')
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      ...
      const timePickerInput = wrapper.find('.ty-input__inner')
  ...
  describe('dismiss events restore picker', () => {
    ...
    it('should be able to focus back and callout picker after clear', async () => {
      ...
      expect(document.querySelector('.ty-time-panel')).toBeFalsy()
      await input.vm.$emit('input', 'a')
      await rAF()
      expect(document.querySelector('.ty-time-panel')).toBeTruthy()
    })
