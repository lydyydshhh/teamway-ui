...
const Comp = {
  template: `
  <ty-popover ref="popoverRef" title="title" :content="content" virtual-triggering trigger="click" />
...
describe('v-popover', () => {
  ...
  test('should render correctly', async () => {
    ...
    expect(document.body.querySelector('.ty-popover').innerHTML).toContain(
      AXIOM
    )
    wrapper.unmount()
  })
  ...
  test('should show popover when reference is mounted', async () => {
    ...
    expect(
      document.body.querySelector('.ty-popover').getAttribute('style')
