...
import installStyle from '@teamway-ui/test-utils/style-plugin'
import {
  TyCheckbox as Checkbox,
  TyCheckboxGroup as CheckboxGroup,
} from '@teamway-ui/components/checkbox'
import Button from '@teamway-ui/components/button/src/button.vue'
...
import type { FormRules } from '@teamway-ui/components/form'
...
describe('Form', () => {
  ...
  it('label width', async () => {
    ...
    expect(findStyle(wrapper, '.ty-form-item__label').width).toBe('80px')
  })
  ...
  it('auto label width', async () => {
    ...
    const formItems = wrapper.findAll<HTMLElement>('.ty-form-item__content')
    ...
    const formItems1 = wrapper.findAll<HTMLElement>('.ty-form-item__content')
  ...
  it('form-item auto label width', async () => {
    ...
    const formItemLabels = wrapper.findAll<HTMLElement>('.ty-form-item__label')
    const formItemLabelWraps = wrapper.findAll<HTMLElement>(
      '.ty-form-item__label-wrap'
    )
  ...
  it('inline form', () => {
    ...
    expect(wrapper.classes()).toContain('ty-form--inline')
  })
  ...
  it('show message', async () => {
    ...
    expect(wrapper.find('.ty-form-item__error').exists()).toBe(false)
  })
  ...
  it('reset field', async () => {
    ...
    expect(wrapper.findAll('.ty-form-item__error')).toHaveLength(0)
    vi.useRealTimers()
  })
  ...
  it('scroll to the exact form option location', async () => {
    ...
    expect(callEl.className).toBe(
      'ty-form-item is-error is-required asterisk-left ty-form-item--label-right'
    )
    window.HTMLElement.prototype.scrollIntoView = oldScrollIntoView
  })
  ...
  it('validate on rule change', async () => {
    ...
    expect(wrapper.find('.ty-form-item__error').text()).toBe('age is: 1')
  })
  ...
  it('should use form disabled input not trigger @blur', async () => {
    ...
    await nextTick()
    const inputWrapper = wrapper.find('.ty-input__wrapper')
  ...
  it('prop is array', async () => {
    ...
    expect(wrapper.find('.ty-form-item__error').text()).toBe(
      'Please input name'
    )
    ...
    await nextTick()
    expect(wrapper.find('.is-error').exists()).toBe(true)
    expect(wrapper.find('.ty-form-item__error').text()).toBe('name is error')
    vi.useRealTimers()
  })
