...
const component = defineComponent({
  extends: TyOption,
  setup(props, ctx) {
    const result = (TyOption.setup as NonNullable<any>)(props, ctx)
    ...
    // `ty-option` will delete the cache before unmount,
    // This is normal for flat arrays `<ty-select><ty-option v-for="3"></ty-select>`,
    ...
    // So the destruction of `ty-option` in `nextTick` will be slower than
    // the creation of new `ty-option`, which will delete the new node,
  ...
  methods: {
    selectOptionClick() {
      // $el.parentElement => ty-tree-node__content
