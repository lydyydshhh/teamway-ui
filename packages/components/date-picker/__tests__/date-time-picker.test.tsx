...
import { TyFormItem } from '@teamway-ui/components/form'
...
describe('Datetime Picker', () => {
  ...
  it('both picker show correct value', async () => {
    ...
    let spinners = document.querySelectorAll(
      '.ty-time-spinner ul li.is-active'
  ...
  it('click now button', async () => {
    ...
    await nextTick()
    ;(
      document.querySelector('.ty-picker-panel__link-btn') as HTMLElement
    ).click()
    await nextTick()
  ...
  it("should date input respect the default format 'YYYY-MM-DD' when format with only time format is setted", async () => {
    ...
    const timeInput = document.querySelector(
      '.ty-date-picker__editor-wrap input'
    )
  ...
  it('time-picker select && input time && input date', async () => {
    ...
    input_.focus()
    await nextTick()
    const timePanel = document.querySelector('.ty-time-panel')
    expect(
      timePanel!.querySelector('.ty-time-spinner')!.innerHTML
    ).not.toBeNull()
    const button: HTMLElement = document.querySelector(
      '.ty-time-panel .confirm'
    )!
    button.click()
    await nextTick()
  ...
  it('now button: can not choose disabled date', async () => {
    ...
    const btn: HTMLElement = document.querySelector(
      '.ty-picker-panel__footer .is-text'
    )!
    btn.click()
    await nextTick()
  ...
  it('confirm button honors picked date', async () => {
    ...
    ;(
      document.querySelector(
        '.ty-date-picker__header .arrow-right'
      ) as HTMLElement
    ).click()
    ;(
      document.querySelector(
        '.ty-date-picker__header .d-arrow-right'
      ) as HTMLElement
    ).click()
    // click confirm button
    ;(
      document.querySelectorAll(
        '.ty-picker-panel__footer .ty-button'
      )[1] as HTMLElement
    ).click()
  ...
  it('selectableRange', async () => {
    ...
    input1.focus()
    await nextTick()
    const list = document.querySelectorAll('.ty-time-spinner__list')
  ...
  it('defaultTime takes effect when the type is datetime', async () => {
    ...
    const someDateTd: HTMLElement = document.querySelector(
      '.ty-picker-panel__content tr:nth-child(3) td:nth-child(4)'
    )!
    const timeInput: HTMLInputElement = document.querySelector(
      '.ty-date-picker__time-header > span:nth-child(2) input'
    )!
  ...
  it('first click accuracy', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const dayItems = document.querySelectorAll('.ty-date-table-cell__text')
...
describe('Datetimerange', () => {
  it('select daterange and default Time and input format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const pickers = document.querySelectorAll('.ty-date-range-picker__content')
    ...
    const pickerss = document.querySelectorAll(
      '.ty-date-range-picker__time-header .ty-date-range-picker__editors-wrap'
    )
    const left = {
      dateInput: pickerss[0].querySelector(
        '.ty-date-range-picker__time-picker-wrap:nth-child(1) input'
      ),
      timeInput: pickerss[0].querySelector(
        '.ty-date-range-picker__time-picker-wrap:nth-child(2) input'
      ),
    }
    const right = {
  ...
  it('select time', async () => {
    ...
    await nextTick()
    const timeInput = document.querySelectorAll(
      '.ty-date-range-picker__editors-wrap input'
    )[1] as HTMLInputElement
    timeInput.blur()
    timeInput.focus()
    timeInput.blur()
    await nextTick()
    const button: HTMLElement = document.querySelector(
      '.ty-date-range-picker__time-picker-wrap .ty-time-panel .confirm'
    )!
    button.click()
    await nextTick()
    const btn = document.querySelectorAll(
  ...
  it('clear button should empty the input value', async () => {
    ...
    const input = wrapper.find('input')
    await input.trigger('focus')
    const dateRow = document.querySelectorAll('.ty-date-table__row')
    ...
    const headerValue = document.querySelectorAll<HTMLInputElement>(
      '.ty-date-range-picker__time-header input'
    )
    ...
    const clearBtn = document.querySelectorAll<HTMLButtonElement>(
      '.ty-picker-panel__footer button'
    )[0]
    clearBtn.click()
    await nextTick()
  ...
  it('selectableRange', async () => {
    ...
    const listLeft = document.querySelectorAll(
      '.ty-date-range-picker__editors-wrap .ty-time-spinner__list'
    )
    ...
    const listRight = document.querySelectorAll(
      '.ty-date-range-picker__editors-wrap.is-right .ty-time-spinner__list'
    )
  ...
  it('select same date, different time', async () => {
    ...
    leftTimeInput.focus()
    await nextTick()
    const leftList = document.querySelectorAll('.ty-time-spinner__list')
    ...
    await nextTick()
    ;(
      document.querySelector('.ty-time-panel__btn.confirm') as HTMLElement
    ).click()
    await nextTick()
    ...
    const rightList = document.querySelectorAll(
      '.is-right .ty-time-spinner__list'
    ) as any
    // auto set left time to right time
    expect(
      rightList[0]!.querySelector('.ty-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[0])
    expect(
      rightList[1].querySelector('.ty-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[1])
    expect(
      rightList[2].querySelector('.ty-time-spinner__item.is-active').innerHTML
    ).toBe(leftSelect[2])
    ...
    ;(
      document.querySelector(
        '.is-right .ty-time-panel__btn.confirm'
      ) as HTMLElement
    ).click()
    await nextTick()
    ;(
  ...
  it('prop defaultTime should not confilt with prop shortcuts', async () => {
    ...
    ;(
      document.querySelector(
        '.ty-picker-panel__sidebar .ty-picker-panel__shortcut'
      ) as HTMLElement
    ).click()
    await nextTick()
  ...
  describe('should not have footer when show-footer is false', () => {
    ...
    it.each(footerAble)(":type='%s'", async (t) => {
      ...
      expect(document.querySelector('.ty-picker-panel__footer')).not.toBeNull()
      showFooter.value = false
      await nextTick()

      expect(document.querySelector('.ty-picker-panel__footer')).toBeNull()
    })
  })
  ...
  it('should left list time be sync with input input change', async () => {
    ...
    const leftTimeInput = document.querySelectorAll<HTMLInputElement>(
      '.ty-date-range-picker__time-picker-wrap input'
    )[1]
    leftTimeInput.value = 'AM 12:00:01'
    triggerEvent(leftTimeInput, 'input')
    await nextTick()
    expect(
      document.querySelectorAll('.ty-time-spinner__list .is-active')[2]
        .textContent
    ).toBe('01')
  })
  ...
  it('should not duplicate panels after confirm left time input', async () => {
    ...
    const pickerss = rangePicker.findAll('.ty-picker-panel__content')
    const cells = pickerss[1].findAll('.available .ty-date-table-cell')
    ...
    await rangePicker.find('.ty-time-panel__btn.confirm').trigger('click')
  ...
  it('should not duplicate panels after confirm right time input', async () => {
    ...
    await input.trigger('focus')
    const rangePicker = wrapper.findComponent(DatePickerRange)
    const cells = rangePicker.findAll('.available .ty-date-table-cell')
    await cells[0].trigger('click')
    await cells[1].trigger('click')
    const pickerss = rangePicker.findAll('.ty-date-range-picker__header')
