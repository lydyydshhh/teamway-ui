...
含有下拉菜单的树形选择器，结合了 `ty-tree` 和 `ty-select` 两个组件的功能。
...
由于这个组件是`ty-tree`和`ty-select`的结合体，他们的原始属性未被更改，故不在此重复。请跳转查看原组件的相应文档。
...
| 属性名     | 详情                                                           | 类型                    | 默认值 |
| ---------- | -------------------------------------------------------------- | ----------------------- | ------ |
| cache-data | 懒加载节点的缓存数据，结构与数据相同，用于获取未加载数据的标签 | ^[array]`CacheOption[]` | []     |
...
| selectedLabel ^(deprecated)       | 获取当前选中的标签                                                                   | ^[object]`ComputedRef<string \| string[]>`                                                                                                                                               |
