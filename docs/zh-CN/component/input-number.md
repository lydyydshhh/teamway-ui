...
:::demo 要使用它，只需要在 `<ty-input-number>` 元素中使用 `v-model` 绑定变量即可，变量的初始值即为默认值。
...
| align ^                     | 内部输入文本对齐                     | ^[enum]`'left' \| 'center' \| 'right'`        | 'center'                |
| disabled-scientific         | 禁用科学计数法的输入（例如输入 'e'） | ^[boolean]                                    | false                   |
