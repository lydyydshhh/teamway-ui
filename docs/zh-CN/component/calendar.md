...
由于 TeamwayUI 的默认语言为中文，如果你需要设置其它的语言，请参考[国际化](/zh-CN/guide/i18n)文档。
...
| controller-type       | 日历头部的控制器类型                                                                                          | ^[enum]`'button' \| 'select'`                                             | button |
| formatter             | 当 `controller-type` 为“select”时的格式标签                                                                   | ^[Function]`(value: number, type: 'year' \| 'month') => string \| number` | —      |
