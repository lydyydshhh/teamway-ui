<template>
  <ty-table-v2 fixed :columns="columns" :data="data" :width="700" :height="400">
    <template #row="props">
      <Row v-bind="props" />
    </template>
  </ty-table-v2>
...
const Row = ({ rowData, rowIndex, cells, columns }) => {
  ...
  if (colSpan > 1) {
    ...
    const style = {
      ...
      width: `${width}px`,
      backgroundColor: 'var(--ty-color-primary-light-3)',
    }
  ...
  if (rowSpan > 1) {
    ...
    const style = {
      ...cell.props.style,
      backgroundColor: 'var(--ty-color-danger-light-3)',
