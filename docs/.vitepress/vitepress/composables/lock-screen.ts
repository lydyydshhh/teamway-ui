...
export const useLockScreen = () => {
  ...
  const cleanup = () => {
    if (!isClient) return
    removeClass(document.body, 'ty-popup-parent--hidden')
  ...
  const lock = () => {
    if (!isClient) return
    withoutHiddenClass = !hasClass(document.body, 'ty-popup-parent--hidden')
    if (withoutHiddenClass) {
    ...
    addClass(document.body, 'ty-popup-parent--hidden')
  }
